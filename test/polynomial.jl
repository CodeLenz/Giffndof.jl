...
@testset "polynomial" begin    
    ...
    function f_polynomial!(t,F::Vector{T}) where T
        ...
        F[1] = 1.0*(t-1.0)^0 + 2.0*(t-1.0)^1 - 3.0*(t-1.0)^2
        F[2] = 2.0*(t-2.0)^0 - 3.0*(t-2.0)^1 
        F[3] = (t-3.0)^2 - 1.0*(t-3.0)^3
    ...
    function Example_polynomial(M,C,K,U0,V0,t0)
        ...
        load_data[1] = [1.0 ; 1.0; 2.0; -3.0]
        load_data[2] = [2.0 ; 2.0; -3.0]
        load_data[3] = [3.0 ; 0.0; 0.0; 1.0; -1.0]
    ...
    # Run the test
    function Run_polynomial(tspan, dt, beta_c, PData::Function )

        # Process the solutions
        T,Y,U = Process( tspan, dt, Example_polynomial, f_polynomial!, PData, beta_c)
    ...
    T,Y,U = Run_polynomial(tspan, dt, beta_c, Problem_Data)
    ...
    T,Y,U = Run_polynomial(tspan, dt, beta_c, PD)
